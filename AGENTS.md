...
- 每个新构建的可执行文件第一次调用 Vision 要编译识别模型，约 30 秒（按代码签名缓存，之后的启动都很快）。应用启动时由 `TextRecognizer.warmUp()` 在后台用一行英文、一行中文预热，预热完成后首次识别只要零点几秒；用空白图或单独一行中英混排来预热只能覆盖一部分，首次识别仍要十几秒。验证方法：把 App 复制一份，用 `codesign -f -s -` 重新签名，得到一个新的可执行文件，再对比有预热和没预热时的首次 `--translate-image` 耗时。Vision 的识别结果偶尔会有波动（曾经有一阵同一张图少识别几行，后来自己恢复了），怀疑识别有问题时多跑几次、对比旧版本，再改代码。
